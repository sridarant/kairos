...
export function buildDecisionObject(astroCtx, personalSeed = 1, daysAhead = 0) {
  ...
  // ── Signals (with PSI and transit enrichment) ────────────────────────────
  const signals = buildSignals(astroCtx)
  // Annotate top PSI planets into positive signals
  if (astroCtx.psi) {
    const topPlanets = Object.entries(astroCtx.psi)
      .filter(([,v]) => v.psi >= 7)
      .sort((a,b) => b[1].psi - a[1].psi)
      .slice(0, 2)
    topPlanets.forEach(([name, data]) => {
      signals.positive.push(`PSI:${name}:${name} is ${data.label} today (PSI ${data.psi})`)
    })
  }
  if (astroCtx.transitContext?.supportive?.length) {
    astroCtx.transitContext.supportive.slice(0,1).forEach(e =>
      signals.positive.push(`TRANSIT:${e.planet}:${e.description}`)
    )
  }
  if (astroCtx.transitContext?.challenging?.length) {
    astroCtx.transitContext.challenging.slice(0,1).forEach(e =>
      signals.caution.push(`TRANSIT:${e.planet}:${e.description}`)
    )
  }
  ...
  return {
    decision,                    // 'DO' | 'WAIT' | 'AVOID'
    confidence: confResult.tier, // 'High' | 'Medium' | 'Low'
    stars:      confResult.stars,
