...
// (fewer confirming factors) and vice versa.

/**
 * rawScoreToSuitability(rawScore)
 * Maps the golden slot composite score (typically -6 to +9) to 0-100.
 * Threshold calibration:
 *   rawScore ≥ 4    → 75-100 (Excellent)
 *   rawScore 2-4    → 55-75  (Good)
 *   rawScore 0-2    → 40-55  (Moderate)
 *   rawScore < 0    → 0-40   (Challenging)
 */
function rawScoreToSuitability(rawScore) {
  // Normalise from [-6,+9] to [0,100] with centre at 0=50
  const normalised = Math.round(50 + rawScore * 5)
