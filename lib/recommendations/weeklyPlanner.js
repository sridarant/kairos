...
export function buildWeeklyPlan(weekPlanArray) {
  ...
  // P0-03 fix: sort by confidence which now represents normalised suitabilityScore
  // (api/daily.js buildWeekPlan was fixed to use rawScore-based suitability instead
  // of the absent confidenceScore which fell back to 50 for every day).
  // This means topDay and worstDay now reflect actual day quality, not a tie at 50.
  const sorted = [...weekPlanArray].sort((a,b) => (b.confidence||0) - (a.confidence||0))
  ...
  const categories = Object.entries(CATEGORY_META).map(([cat, meta], idx) => {
    ...
    const isGood = (best.confidence || 50) >= 55
    return {
      category:   cat,
      icon:       meta.icon,
      label:      meta.label,
      ...
      confidence: (best.confidence || 50) >= 70 ? 'High' : (best.confidence || 50) >= 45 ? 'Medium' : 'Low',
