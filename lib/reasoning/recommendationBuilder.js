...
// Per-category best window calculation
// Maps category to the slot dimension most relevant to it
const CAT_DIM = {
  career:        { dim:'d', invert:false },
  finance:       { dim:'r', invert:true  },   // lower risk = better
  relationships: { dim:'c', invert:false },
  health:        { dim:'f', invert:false },
  education:     { dim:'f', invert:false },
...
/**
 * bestWindowForCategory(cat, scoredSlots, goldenWindow)
 * Returns the slot time with the highest value for the category's primary dimension.
 * Falls back to goldenWindow if scoredSlots is empty or all slots are uniform.
 */
function bestWindowForCategory(cat, scoredSlots, goldenWindow) {
  const cfg = CAT_DIM[cat]
  if (!cfg || !scoredSlots?.length) return goldenWindow
  ...
  // If all slots have identical values on this dimension, the golden window
  // is a genuine result — return it (do not fabricate variation)
  const top = sorted[0]
  if (sorted.every(s => s.dims[cfg.dim] === top.dims[cfg.dim])) return goldenWindow
  return top.time
}

export function buildRecommendations(reasoningResult, goldenWindow, evidenceNodes, scoredSlots = []) {
  ...
  for (const [cat, catReason] of Object.entries(categories)) {
    ...
    recs.push({
      ...
      action,
      reason:     reasonText,
      bestTime:   bestWindowForCategory(cat, scoredSlots, goldenWindow),
