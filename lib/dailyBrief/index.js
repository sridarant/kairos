...
/**
 * deriveOutlook(suitabilityTier, decision)
 *
 * P0-01 fix: outlook now derived from suitabilityTier (day quality),
 * not confidence (calculation certainty). Previously a low-confidence but
 * high-suitability day was labelled 'Challenging' — which was incorrect.
 */
function deriveOutlook(suitabilityTier, decision) {
  if (suitabilityTier === 'Excellent')  return 'Positive'
  if (suitabilityTier === 'Good')       return 'Positive'
  if (suitabilityTier === 'Challenging')return 'Challenging'
...
function buildTomorrowPreview(weekPlan) {
  ...
  return {
    ...
    confidence: conf,
    bestWindow: tomorrow.bestWindow || null,   // populated if API provides it
    theme:      tomorrow.theme     || null,
    days_ahead: 1
  }
}
...
export function buildMorningBrief(daily, primaryMember) {
  ...
  // P0-01 fix: use suitabilityTier for outlook label, not confidence tier
  const suitabilityTier = decisionObj.suitabilityTier || 'Neutral'
  const outlook         = deriveOutlook(suitabilityTier, decision)

  const brief = {
    theme:          primaryMember.focus || daily.focus || 'Decision Making',
    outlook,
    suitabilityScore: decisionObj.suitabilityScore ?? null,
    suitabilityTier,
    confidenceScore:  decisionObj.confidenceScore ?? null,
