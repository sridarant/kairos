...
import { addHistory, updateOutcome, trackAsk } from '../lib/dataClient'

const COLORS = {
  do:    { bg: 'var(--green-bg)', txt: 'var(--green-txt)' },
  avoid: { bg: 'var(--red-bg)',   txt: 'var(--red-txt)'   },
...
export default function AskModal({ onClose, profile, feedbackAdj }) {
  ...
  async function handleFeedback(outcome) {
    setFeedback(outcome)
    if (entryId) await updateOutcome(entryId, outcome, actionTaken)
  }

  async function handleActionTaken(acted) {
    setActionTaken(acted)
    if (entryId) await updateOutcome(entryId, feedback, acted)
  }

  function handleAskAnother() {
    setResult(null); setFeedback(null); setActionTaken(null)
    setEntryId(null); setQuestion(''); setContext('')
  }
          ...
          {!result ? (
            ...
            <div className="fade-in">
              ...
              <div style={{ background: colors.bg, borderRadius: 12, padding: 16, marginBottom: 12 }}>
                ...
                {result.action && (
                  <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 8, color: 'var(--white)' }}>
                    {result.action}
                  </p>
                )}
                <p style={{ fontSize: 14, lineHeight: 1.5 }}>{result.message}</p>
                {result.balance && (
                  <p style={{ fontSize: 12, color: colors.txt, marginTop: 10, opacity: 0.8, lineHeight: 1.4 }}>
                    {result.balance}
                  </p>
                )}
              </div>
              ...
              {actionTaken === null ? (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {smBtn('✓ Yes', () => handleActionTaken(true))}
                    {smBtn('✗ No',  () => handleActionTaken(false))}
                  </div>
                </div>
              ) : (
              ...
              {!feedback ? (
                <div style={{ marginBottom: 14 }}>
                  <p style={{ fontSize: 12, color: 'var(--gray-4)', marginBottom: 8 }}>Was this helpful?</p>
                  <div style={{ display: 'flex', gap: 8 }}>
                    {smBtn('👍 Yes', () => handleFeedback('success'))}
                    {smBtn('👎 No',  () => handleFeedback('fail'))}
                  </div>
                </div>
              ) : (
                <p className="fade-in" style={{ fontSize: 12, color: 'var(--gray-4)', marginBottom: 14, textAlign: 'center' }}>
